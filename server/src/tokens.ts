...
/**
 * Chave de leitura dos arquivos do talk.
 *
 * Imagem e link de PDF nao mandam cabecalho `Authorization`: um `<img src>` e
 * um `<a href>` so conhecem o endereco. Entao a chave vai no endereco - e por
 * isso ela nao e o token de sessao. Serve so para baixar arquivo, nao abre o
 * PostgREST (nao carrega `role: 'authenticated'`), e quem decide se a pessoa
 * alcanca aquele arquivo continua sendo o banco, na hora do download.
 *
 * O vencimento e arredondado para uma janela de doze horas, e o token nasce
 * sem `iat`: a mesma pessoa recebe a mesma chave o dia inteiro, o endereco da
 * capa nao muda a cada abertura do app, e o navegador consegue guardar a
 * imagem em vez de baixar de novo.
 */
const JANELA_CHAVE_SEGUNDOS = 12 * 60 * 60

export function signFileKey(user: { id: string; email: string }): {
  chave: string
  expiraEm: number
} {
  const agora = Math.floor(Date.now() / 1000)
  const expiraEm = (Math.floor(agora / JANELA_CHAVE_SEGUNDOS) + 2) * JANELA_CHAVE_SEGUNDOS
  const chave = jwt.sign(
    { sub: user.id, email: user.email, scope: 'arquivos', exp: expiraEm },
    config.jwtSecret,
    { algorithm: 'HS256', noTimestamp: true },
  )
  return { chave, expiraEm }
}

export function verifyFileKey(chave: string): SessionClaims | null {
  try {
    const payload = jwt.verify(chave, config.jwtSecret, { algorithms: ['HS256'] })
    if (typeof payload === 'string' || payload.scope !== 'arquivos' || !payload.sub) return null
    return { sub: String(payload.sub), email: String(payload.email ?? ''), role: 'authenticated' }
  } catch {
    return null
  }
}
